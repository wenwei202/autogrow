...
class BasicSwitchBlock(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1):
        super(BasicSwitchBlock, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)
        self.switch = pswitch.PSwitch()
...
def PlainNoBNNet(num_blocks, num_classes=10, image_channels=3):
    assert len(num_blocks) == 4, "4 blocks are needed, but %d is found." % len(num_blocks)
    # ResNet is NOT a ResNet, it's just a building func
    return ResNet(PlainNoBNBlock, num_blocks, num_classes=num_classes, image_channels=image_channels, batchnorm=False)
